...
public class Drivetrain {
    ...
    */
    ...
    public DistanceSensor rDistance, lDistance;
    ...
    */
    ...
    public void close_pixel() {
        pixel.setPosition(0.0);
        pixel2.setPosition(1.0);
    }
    ...
    */
    private void initHardwareMap(HardwareMap hwMap) {
        ...
        pixel = hwMap.servo.get("pixel");
        pixel2 = hwMap.servo.get("pixelB");
        drone = hwMap.servo.get("drone");
        hook = hwMap.servo.get("hook");
        initIMU();
        lDistance = hwMap.get(DistanceSensor.class, "leftDistance");
        rDistance = hwMap.get(DistanceSensor.class, "rightDistance");
    ...
    */
    ...
    public void rotate(boolean left, boolean right, double power){
    ...
    */
    public void orientVertical(double bDist) throws InterruptedException{ 
        ...
        while (Math.abs(currentbDistance - bDist) > 1){
            ...
            currentbDistance = rDistance.getDistance(DistanceUnit.CM);
        } 
    ...
    */ 
    public void drive
    (
        Drivetrain robot, Claw claw, Wrist wrist, Arm arm, boolean gamepad2A, boolean gamepad2B, boolean gamepad2X, boolean gamepad2Y,
        ...
        boolean gamepad1LB, boolean gamepad1RB, boolean gamepad1DPU, boolean gamepad1DPD,boolean gamepad2DPU,boolean gamepad2DPD, BNO055IMU imu
    )
    
    {
        //robot.changeSpeed(gamepad2X);
        
        if(lastX == true && gamepad2X == false){
            changeSpeed(true);
        }
        lastX = gamepad2X;
        if(gamepad1A){
        ...
        if (gamepad2DPR || gamepad2DPL){
              robot.strafe(gamepad2DPL, gamepad2DPR);
        }
        if (gamepad2DPU || gamepad2DPD){
              robot.forward(gamepad2DPD, gamepad2DPU);
        }
            
        if (gamepad2RB|| gamepad2LB){
            //robot.rotate(gamepad2RB, gamepad2LB);
        }
        // claw.setClawPosin(gamepad1A);
        // wrist.setWristPosin(gamepad1B);
        claw.setClawPosin(gamepad2A);
        wrist.setWristPosin(gamepad2B);
        
        arm.armMoveIn(gamepad2RB, gamepad2LB);
        //arm.armMoveIn(gamepad1DPD, gamepad1DPU);
        // if (gamepad2RB|| gamepad2LB){
        //     arm.armMoveIn(gamepad2RB, gamepad2LB);
        // }
        //Move arm up to score
        if(gamepad2Y){
            arm.armMoveDist(100, 1);
        }
        ...
        if (gamepad1DPD || gamepad1DPU){
            arm.armMove(gamepad1DPU, gamepad1DPD);
        }
